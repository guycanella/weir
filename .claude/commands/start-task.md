...
# Start Task — WR-NNN Delivery Workflow
...
- Dispatch the implementer by artifact ownership:
  ...
  - **bruce** — the doc triad (`DOCUMENTATION.md`/`IMPLEMENTATION.md`/README) and ADRs.
...
- Fix any blocking internal findings by dispatching them to the specialist who owns the affected artifact (see step 3's ownership list — a Go finding goes to julia, a manifest/RBAC finding to john, a `.tf` finding to viktor, a test-coverage finding to flynn, a docs/ADR finding to bruce, a security finding to lisa) before the external gate. A single task can span more than one owner — route each finding individually, don't default everything to one "the implementer".
...
- Dispatch **lisa** for the final scan (RBAC review, trivy; for `[stretch]` supply-chain: cosign + SBOM). A `high` finding sends you back to step 3 (route it to the owning specialist, as above).
