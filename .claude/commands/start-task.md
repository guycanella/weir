...
5. **Finalize on PASS only.** Lisa's security scan → a Conventional Commit with a `Refs: $ARGUMENTS` footer → open the PR → mark `DONE` in `PROGRESS.md` with a summary, which agents did what, and which model reviewed.
