...
1. **Branch check.** `PROGRESS.md` is gitignored, so a fresh clone won't have it — if it's absent, create it first with the standard status-board + log skeleton, treating this as an empty progress state. Then read `PROGRESS.md`: if `WR-NNN` is already `IN PROGRESS` there with a recorded branch, the current branch matches it, and the row's Notes are anything other than `awaiting merge`, this is a resumed mid-work session — continue from wherever the log left off; being off `main` is expected, not an error. If Notes **are** `awaiting merge`, the work is already prepared — do not redispatch specialists; go straight to asking the human whether the PR merged (step 7). Otherwise (a fresh start, no matching entry): if on `main`, create and switch to the task branch `<type>/WR-NNN/<scope>` (type from the task's nature; scope a kebab-case slug from the task description). If on neither `main` nor the recorded resume branch, stop and ask the human to switch first — never silently adopt an unrelated branch as the task branch.
...
8. On **FAIL**: log findings. `high` severity blocks and is always routed for a fix; `medium` is normally worth fixing too (established project practice), at your judgment; `low` is a note only — log it and move on, never dispatch a specialist or re-run the gate just for a low finding. For blocking findings, route each one to the specialist who owns the affected artifact — Go code to the go specialist, CRD/reconcile/RBAC/KEDA/Helm to the kubernetes specialist, `.tf`/LocalStack toggle to the terraform specialist, test-coverage gaps to the tester, security findings to the security specialist, docs/ADR findings to the docs specialist — rather than a single generic "the coder"; a task can span more than one owner. Re-review the diff only. **Cap at 3 iterations**; on the cap, stop and escalate to the human.
