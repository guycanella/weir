...
#   - Revisit this if/when we adopt Tilt's `k8s_context()`/
#     `allow_k8s_contexts()` guards to fail fast with a clearer message,
#     or restructure so kind-cluster creation happens outside Tilt's
